...
class LessonViewModel with ChangeNotifier {
  ...
  // to determine what text should appear in individual_lesson_view button to complete a lesson
  // if not yet complete displays cash user can earn, otherwise plain string
  String lessonCompletionButtonText(LessonModel lesson) {
    if (!lesson.getComplete()!) {
      return ("Reward for completing: \$" + lesson.getCash()!.toString());
    }
    return "Lesson is Complete!";
  }
}
...
class LessonDatabase {
  ...
  Future<Database> get database async {
    ...
    instance.insertLesson(new LessonModel(
        ...
        false,
        "The two main types of stock are: common stock and preferred stock. When most people talk about stocks, they are mostly reffering to the common stock. Most stock are in the form of common stock. The common stock is essentially a common share that represents ownership in a company and a claim on a portion of profits. Over long term, the common stock, yields higher retunrs than almost any other investment.  The preffered stock represents some degree of ownership in a company but does not come with the same voting rights. withe preferred shares investors are guaranteed a fixed divident forever. This is different from common stock, which does not guarnatee dividends. There are many other different types of stocks but these are the most important and relevant ones to new investors.  ",
        10));
    instance.insertLesson(new LessonModel(
        "Lesson 4!",
        "The Golden Rule: Sell High, Buy Low ",
        'assets/images/lesson4.png',
        false,
        "The Golden Rule of trading is to Sell High, Buy Low! This means to make sure to sell your stocks at high points in the market and to buy them at low points. The stock market is a waiting game. You need to always stay updated about the recents changes in each stock in order to truly get the most out of your investments. When the stock market is low, buy more stocks. When the stock market is high, sell your stocks. Remember if you did not sell your stocks on a high just wait for the next one. The stock market will crash and come back again like it has many times before. Just be patient!  ",
        10));
    instance.insertLesson(new LessonModel(
        "Lesson 5!",
  ...
  Future _createDB(Database db, int version) {
    return db.execute("CREATE TABLE lessons("
        "'title' TEXT PRIMARY KEY,"
        "'description' TEXT,"
        "'image' TEXT,"
        "'complete' TEXT,"
        "'content' TEXT,"
        "'cash' INTEGER)" //, description TEXT, content TEXT, complete BOOLEAN, image TEXT)',
        );
  }
